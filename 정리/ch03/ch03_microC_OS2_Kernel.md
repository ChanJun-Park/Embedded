...
```c
OSRdyTbl[prio >> 3] |= OSMapTbl[prio & 0x70];
```
...
```c
if ( (OSRdyTbl[prio >> 3] &= ~OSMapTbl[prio & 0x70]) == 0) {
    OSRdyGrp &= ~OSMapTbl[prio >> 3];
}
```
...
이를 이진수로 변환하면 11100100 이 된다. 맨 마지막에서 **2**번째 비트가 1로 셋팅되어 있다. 즉, 이 비트에 해당하는 task가 우리가 찾는 task 되시겠다. 다시 컴퓨터가 이 비트를 찾게 하려면 `OSUnMapTbl[0xE4]`를 확인하면 된다. 이는 **2**다. 위에서 찾은 **2**와 동일하다.
